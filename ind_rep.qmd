...
**Figure 1. Weekdays vs. Weekend Patterns.** Hourly ridership was compared between weekdays and weekends to distinguish commute-oriented usage from leisure-oriented travel, revealing weekday peaks and flatter weekend trends.
...
**Figure 2. Hour x Day-of-Week Heat Map.** This heat map was used to visualize hourly and weekly ridership intensity for Manhattan. Weekends display strong daytime and commute peaks, while weekends exhibit flatter, more evenly distributed ridership patterns consistent with non-commuting travel.
...
**Table 1. Summary Statistics.** Highlights differences between weekend and weekday usage in Manhattan.
...
**Figure 3. Regression Coefficients for Hour-of-day and Weekend Effects.** Coefficient estimated from a linear regression of hourly Citi Bike trip counts in Manhattan, using hour-of-day fixed effects and a weekend indicator.
...
**Table 2. Regression Table Results.** Estimate the effect of weekend status hourly Citi Bike ridership in Manhattan, while controlling for hour-of-day fixed effects.
...
Figure 3 displays regression coefficients from a linear model. Based on this plot, we can interpret that, relative to the midnight serving as a baseline, ridership increases substantially during daytime hours, with the largest positive effects observed during commuting and midday periods. There’s a sharp rise in coefficients during early morning, peak late morning and early afternoon, and then gradually decline into the evening.  
...
- Approximately of variation `{r} paste0(format(round(r2 * 100, 1), big.mark = ","), "%")` in hourly ridership is explained by the variables of time-of-day and weekend status.  
